...
class SQLPlugin:
    ...
    async def generatePDF(self):
        ...
        await cl.Message(
            content="Download the file: ", elements=elements
        ).send()
...
class ProductSearchPlugin:
    ...
    async def search_products(self, product_name: str, quantity: str = "1") -> str:
        ...
        for result in results:
            ...
            # Check if product meets order requirements
            # requested_qty = int(float(quantity))
            # product["can_order"] = (
            #     (product["max_order_quantity"] == 0 or requested_qty <= product["max_order_quantity"]) and
            #     requested_qty <= product["available_quantity"]
            # )
...
async def initialize_mcp_plugin():
    async with MCPStdioPlugin(
        name="Demo Server",
        description="Plugin for demo server",
        command="dotnet",
        args=[
            "run",
            "--project",
            "C:\\Users\\ldas009\\Downloads\\RAG Hack\\MCPFirst\\MCPFirst\\MCPFirst.csproj",
            "--no-build",
        ],
    ) as pluginmc:
        return pluginmc
...
async def on_chat_start():
    ...
    pluginmcp = await initialize_mcp_plugin()
    print("Connecting to MCP server MCPFirst...")
    await pluginmcp.connect()
    print("MCP server connected.")
    kernel.add_plugin(pluginmcp, "MCPFirst")
...
IMPORTANT: Do Not Generate Random Values provide information that matches the user's query""")
    ...
    name="QueryAgent",
    instructions=("""
You are expert in SQL query.Your task is to INSERT invoice data into an SQL database using the insert_invoice function of the InsertQuery plugin.
*AFTER  YOU INSERT Invoice data in sql databse MUST ALWAYS generate Invoice PDF using function GenerateInvoicePdf of MCPFirst ,After that USE  generatepdf function of InsertQuery plugin and don't send aurgument*
...
"concat": "Example Product 1, Quantity: 2, UnitPrice: 10.00 Example Product 2, Quantity: 5, UnitPrice: 20.00 110.00 2023-10-07"
    ...
    router_agent = ChatCompletionAgent(
        ...
        instructions=(
        """
You are a helpful AI assistant named GROOT who responds with warm and welcoming tone.If you are unsure about an answer, it's okay to say you don't know rather than guessing.
Your role is to efficiently direct user inquiries to the agents and understand the user's question and identify the INTENT behind it.

The role of different agents are described:

ProductAgent manage Product information
Keywords to identify: "product info," "what is," "tell me about," "price of"

InvoiceAgent provide previous order information
Keywords to identify: "previous order," "order details"

OrderAgent operates for placing order and calls QueryAgent
...
Guidelines:
    Context Maintenance: Always Keep track of the conversation context for seamless interactions and to complete the users requirement
    Delegation: Always delegate queries to the relevant specialist agents
    Data Limitation: DO NOT provide response from other resource, stick to the product knowledge connected with PorductSearch
    Always maintain the ongoing session context
...
If a query cannot be addressed with the available data, respond with "I don't know" rather than attempting to generate information."""
        ...
        plugins=[product_agent,order_agent,fetch_orders_agent]
    )
...
async def chat_profile(current_user: cl.User):
    ...
    return [
        cl.ChatProfile(
            name="Groot",
            # icon="https://static.vecteezy.com/system/resources/previews/020/662/325/large_2x/store-icon-logo-illustration-vector.jpg",
            ...
            starters=[
                ...
                #icon=r"C:\\Users\\smitra208\\OneDrive - pwc\\Desktop\\InsuranceChatbot\\insurance_flow\\src\\insurance_flow\\idea-svgrepo-com.svg",
                ),
